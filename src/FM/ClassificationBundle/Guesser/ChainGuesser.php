    ...
    protected function guessCumulative($value)
    {
        ...
        foreach ($this->guessers as list($guesser, $boost, $inputNormalizer, $outputNormalizer)) {
            ...
            $this->reportData[] = [
                ...
                'top' => !is_object($guesses->top()) ? var_export($guesses->top(), true) : get_class($guesses->top()) . ':' . json_encode($guesses->top()),
                'results_topScore' => $results->topScore(),
                'results_top' => !is_object($results->top()) ? var_export($results->top(), true) : get_class($results->top()) . ':' . json_encode($results->top()),
                'max' => $max,
                'boost' => $boost,
